public class Queen extends ChessPiece {
    ...
    public boolean canMoveToPosition(ChessBoard chessBoard, int line, int column, int toLine, int toColumn) {
        ...
        if ((Math.abs(column - toColumn) != Math.abs(line - toLine)) && (line != toLine && column != toColumn)) {
            return canMove; //move only diagonally or vertically
        }

        if (line == toLine || column == toColumn) { //queen is moving in a straigth line
            if (!noFiguresInTheWayInAStraightLine(chessBoard, line, column, toLine, toColumn)) {
                return canMove;
            }
        } else if (Math.abs(column - toColumn) == Math.abs(line - toLine)) { //queen is moving in a diagonally
